...
{
	//-------------------------------------------------------------------------
	public class CustomFireSource : MonoBehaviour
	{
        public SteamVR_Action_Boolean startMagicAction;
		...
		void Update()
		{
            bool state = SteamVR_Actions._default.TouchpadPress[SteamVR_Input_Sources.RightHand].state;

            if (state && !isBurning) StartBurning();
            else if (!state) StopBurning();
        }
